...
objective_function = @(ecC_int_value) calc_percent_diff(input_file_raw,T, ecC_bulk, ecC_int_value, ecC_eff_exp, ...
                                                        moose_executable, input_file_dir, input_file, ...
                                                        output_csv_path, len_scale);

% Initial guess for ecC_int
initial_guess = 8.83e4; % Example initial guess

% Perform the optimization using fminsearch
options = optimset('TolFun', 0.05,'TolX',15, 'MaxIter',15, 'MaxFunEvals',100,'Display','iter'); % Set tolerance and limits
% [optimal_ecC_int, final_diff] = fmincon(objective_function, initial_guess, [],[], [],[],[],[],[], options);
...
function percent_diff = calc_percent_diff(input_file_raw,T, ecC_bulk, ecC_int_value, ecC_eff_exp, ...
    ...
    modify_input_file(input_file_raw,T, ecC_bulk, len_scale, ecC_int_value, input_file_dir, input_file);

    % Run the MOOSE job
    run_moose_job(moose_executable, input_file, input_file_dir);
...
function modify_input_file(input_file_raw,T, ecC_bulk,len_scale,ecC_int_value,input_file_dir, input_file)
...
function run_moose_job(moose_executable, input_file, input_file_dir)
%     command = sprintf('mpiexec -n 2 %s -i %s', moose_executable, input_file);
%     activate_moose = sprintf('conda activate moose');
%     system(activate_moose)
    command = sprintf('%s -i %s', moose_executable, input_file);% Change processor count as needed
...
function ecC_eff = calculate_ecC_eff(output_csv_path, len_scale)
    ...
    qx_ave = data_out{1, 4}; % Adjust column index as per your CSV
    qy_ave = data_out{1, 5}; % Adjust column index as per your CSV
    left_V = data_out{1, 3}; % Adjust column index as per your CSV
    right_V = data_out{1, 6}; % Adjust column index as per your CSV

    % Calculate the difference in voltage and current density
    dV = right_V - left_V;
    dx = 160e-6; % mm unit 2D dimension, for 3D = 80e-6 mm
    j_tot = sqrt(qx_ave^2 + qy_ave^2);

    % Compute ecC_eff
    ecC_eff = -j_tot * dx / (len_scale * dV); % Replace with your specific formula
end
